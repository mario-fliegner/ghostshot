...
class GuideNavigationTest {
    ...
    @Test
    fun guideTopic_opensDetailAndBackReturnsToGuide() {
        setGuideNavigationContent()

        composeRule.onNodeWithTag("guide_topic_reference_photos").performClick()
        composeRule.waitForIdle()
        composeRule.onNodeWithTag("guide_detail_root").assertIsDisplayed()

        composeRule.onNodeWithTag("guide_detail_back_button").performClick()
        composeRule.waitForIdle()
        composeRule.onNodeWithTag("guide_screen_root").assertIsDisplayed()
    }

    @Test
    fun showWalkthroughAgain_opensWalkthroughAndSkipReturnsToGuide() {
        setGuideNavigationContent()
    ...
    private fun setGuideNavigationContent() {
        wakeTestDevice()
        ...
        scenario?.onActivity { activity ->
            ...
            activity.setContent {
                SameViewTheme {
                    ...
                    NavHost(navController = navController, startDestination = ROUTE_GUIDE) {
                        ...
                        composable(
                            ...
                            WalkthroughScreen(
                                entryMode = entryMode,
                                windowWidthSizeClass = WindowWidthSizeClass.Compact,
                                onSkip = { navController.popBackStack(ROUTE_GUIDE, inclusive = false) },
                                onStart = { navController.popBackStack(ROUTE_GUIDE, inclusive = false) }
                            )
                        }
                    }
                }
